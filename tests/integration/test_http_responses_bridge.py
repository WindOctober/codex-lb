...
async def _collect_sse_events(
    ...
    *,
    ...
    async with async_client.stream("POST", path, json=json_body, headers=headers) as response:
        ...
        lines = [line async for line in response.aiter_lines() if line.startswith("data: ") and line != "data: [DONE]"]
...
def _install_proxy_settings(
    ...
    *,
    ...
    monkeypatch.setattr(proxy_api_module, "get_settings", lambda: app_settings)
...
class _DelayedFirstEventUpstreamWebSocket(_FakeBridgeUpstreamWebSocket):
    ...
    async def send_text(self, text: str) -> None:
        async def emit() -> None:
            ...
            await super(_DelayedFirstEventUpstreamWebSocket, self).send_text(text)

        self._emit_task = asyncio.create_task(emit())
...
class _SelectedModelCapacityUpstreamWebSocket(_FakeBridgeUpstreamWebSocket):
    async def send_text(self, text: str) -> None:
        self.sent_text.append(text)
        await self._messages.put(
            _FakeUpstreamMessage(
...
class _ServerOverloadedUpstreamWebSocket(_FakeBridgeUpstreamWebSocket):
    async def send_text(self, text: str) -> None:
        self.sent_text.append(text)
        await self._messages.put(
            _FakeUpstreamMessage(
                ...
                text=json.dumps(
                    {
                        ...
                        "response": {
                            "id": "resp_bridge_server_overloaded",
                            "object": "response",
                            "status": "failed",
                            "error": {
                                "type": "server_error",
...
def _make_dummy_bridge_session(session_key: proxy_module._HTTPBridgeSessionKey) -> proxy_module._HTTPBridgeSession:
    ...
    return proxy_module._HTTPBridgeSession(
        key=session_key,
        headers={},
        affinity=proxy_module._AffinityPolicy(key=session_key.affinity_key),
...
@pytest.mark.asyncio
async def test_backend_http_bridge_returns_terminal_sse_when_startup_fails_after_keepalive(
    async_client,
    monkeypatch,
):
    _install_bridge_settings(monkeypatch, enabled=True)
    monkeypatch.setattr(proxy_module, "_ACCOUNT_SELECTION_RECOVERY_HEARTBEAT_SECONDS", 0.01)
    monkeypatch.setattr(proxy_module, "_HTTP_BRIDGE_STARTUP_KEEPALIVE_GRACE_SECONDS", 0.01)

    async def fail_after_keepalive(self, *args, **kwargs):
    ...
    events = await _collect_sse_events(
        ...
        headers={"x-codex-session-id": "session-startup-failure-after-keepalive"},
    )

    assert any(event.get("type") == "codex.keepalive" for event in events)
...
async def test_v1_responses_http_bridge_reconnect_uses_refreshed_api_key_assignments_for_reused_session(
    ...
    await service._release_http_bridge_submit_lease(bridge_session)
    ...
    await service._release_http_bridge_submit_lease(reused_session)
...
async def test_v1_responses_http_bridge_prefers_evicting_prompt_cache_session_before_codex_session(
    ...
    await service._release_http_bridge_submit_lease(codex_session)
    ...
    await service._release_http_bridge_submit_lease(prompt_session)
    ...
    await service._release_http_bridge_submit_lease(created)
...
async def test_backend_responses_http_emits_turn_state_header_and_reuses_when_replayed(
    ...
    assert len(connect_calls) == 1
    assert connect_calls[0][0] is not None
    assert connect_calls[0][0].startswith("prompt-cache:v2:")
    assert connect_calls[0][1] == proxy_module.StickySessionKind.PROMPT_CACHE
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_emits_keepalive_before_delayed_first_event(async_client, monkeypatch):
    _install_bridge_settings(monkeypatch, enabled=True)
    account_id = await _import_account(
        async_client,
        "acc_http_bridge_delayed_first_event",
        "http-bridge-delayed-first-event@example.com",
    )
    account = await _get_account(account_id)
    upstream = _DelayedFirstEventUpstreamWebSocket(delay_seconds=0.05)
    ...
    events = await _collect_sse_events(
        ...
        json_body={
            "model": "gpt-5.6-sol",
            "instructions": "Return exactly OK.",
            "input": "delayed-first-event",
            "prompt_cache_key": "http-bridge-delayed-first-event",
            "stream": True,
        },
    )

    assert events[0]["type"] == "codex.keepalive"
    assert [event["type"] for event in events if event["type"] != "codex.keepalive"] == [
        "response.created",
        "response.completed",
    ]
...
async def test_backend_responses_http_bridge_refresh_failure_returns_proxy_error(async_client, monkeypatch):
    ...
    assert response.json()["error"]["message"] == "No active accounts available"
    assert "token expired" not in response.text
    assert "x-codex-turn-state" not in response.headers
...
async def test_v1_responses_http_bridge_does_not_register_turn_state_alias_before_request_admission(
    ...
    async def fake_submit_http_bridge_request(
        ...
        *,
        ...
        del self, session, request_state, text_data, queue_limit, release_submit_lease
        raise proxy_module.ProxyResponseError(
            429,
            proxy_module.openai_error(
                "rate_limit_exceeded",
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_does_not_replay_completed_send_when_upstream_closes_before_created(
    async_client,
    monkeypatch,
):
    _install_bridge_settings(monkeypatch, enabled=True)
...
@pytest.mark.parametrize(
    "failing_upstream_factory,prompt_cache_key",
    [
        (_SelectedModelCapacityUpstreamWebSocket, "model-capacity-failover-key"),
        (_ServerOverloadedUpstreamWebSocket, "server-overloaded-failover-key"),
    ],
)
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_fails_over_on_precreated_capacity_error(
    async_client,
    monkeypatch,
    failing_upstream_factory,
    prompt_cache_key,
):
    _install_bridge_settings(monkeypatch, enabled=True)
    first_account_id = await _import_account(
        async_client,
        "acc_http_bridge_model_capacity_a",
        "http-bridge-model-capacity-a@example.com",
    )
    second_account_id = await _import_account(
        async_client,
        "acc_http_bridge_model_capacity_b",
        "http-bridge-model-capacity-b@example.com",
    )
    first_account = await _get_account(first_account_id)
    second_account = await _get_account(second_account_id)
    upstreams = [failing_upstream_factory(), _FakeBridgeUpstreamWebSocket()]
    ...
    async def fake_select_account_with_budget(
        ...
        *,
        ...
        held_http_bridge_session_account_id=None,
        request_stage=None,
    ):
        del (
            self,
            ...
            held_http_bridge_session_account_id,
            request_stage,
        )
        account = first_account if not selected_account_ids else second_account
    ...
    assert selected_account_ids == [first_account_id, second_account_id]
    assert connect_count == 2
...
async def test_v1_responses_http_bridge_does_not_evict_queued_session_when_pool_is_full(
    ...
    first_response_create_gate = asyncio.Semaphore(1)
    await first_response_create_gate.acquire()
    first_session.response_create_gate = first_response_create_gate
    ...
    async with asyncio.timeout(_TEST_SYNC_TIMEOUT_SECONDS):
        while await service._http_bridge_pending_count(first_session) != 1:
            await asyncio.sleep(0)
    ...
    with pytest.raises(asyncio.CancelledError):
        await submit_task
    first_response_create_gate.release()
    await service._close_http_bridge_session(first_session)
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_default_queue_limit_zero_allows_more_than_legacy_cap(
    async_client,
    app_instance,
    monkeypatch,
):
    ...
    account_id = await _import_account(
        async_client,
        "acc_http_bridge_queue_unlimited",
        "http-bridge-queue-unlimited@example.com",
    )
    service = get_proxy_service_for_app(app_instance)
    account = await _get_account(account_id)
    hanging_upstream = _SilentUpstreamWebSocket()
    ...
    payload = proxy_module.ResponsesRequest(
        ...
        instructions="Return exactly OK.",
        input="same-session",
        prompt_cache_key="same-session-unlimited-key",
    )
    affinity = proxy_module._sticky_key_for_responses_request(
        payload,
        {},
    ...
    key = proxy_module._make_http_bridge_session_key(
        ...
        affinity=affinity,
        api_key=None,
        request_id="req_queue_unlimited",
    )
    session = await service._get_or_create_http_bridge_session(
        key,
        headers={},
    ...
    try:
        ...
        assert session.response_create_gate is None
        assert session.queued_request_count == 12
        async with session.pending_lock:
            assert len(session.pending_requests) == 12
        assert len(hanging_upstream.sent_text) == 12
    finally:
        await service._close_http_bridge_session(session)
...
async def test_v1_responses_http_bridge_singleflight_follower_replaces_session_when_account_is_no_longer_assigned(
    ...
    try:
        ...
        assert follower_session.key != key
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_ambiguous_send_failure_is_not_replayed(async_client, monkeypatch):
    _install_bridge_settings(monkeypatch, enabled=True)
    account_id = await _import_account(
        async_client,
    ...
    assert response.status_code == 502
    assert response.json()["error"]["code"] == "upstream_unavailable"
    assert response.json()["error"]["message"] == "HTTP bridge upstream send failed"
    assert "socket closed" not in response.json()["error"]["message"]
    assert connect_count == 1
    assert len(upstreams[0].sent_text) == 1
    assert upstreams[1].sent_text == []
...
async def test_retry_http_bridge_precreated_request_releases_pending_lock_before_reconnect(app_instance, monkeypatch):
    ...
    async def fake_reconnect(
        ...
        *,
        ...
        assert prefer_same_account is False
        assert require_exclusive_request is True
        reconnect_started.set()
        await _wait_for_event(allow_reconnect_finish)
        target_session.upstream = replacement_upstream

    monkeypatch.setattr(proxy_module.ProxyService, "_reconnect_http_bridge_session_locked", fake_reconnect)
...
@pytest.mark.asyncio
async def test_retry_http_bridge_precreated_request_refuses_when_sibling_is_pending(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    session = proxy_module._HTTPBridgeSession(
...
@pytest.mark.asyncio
async def test_retry_http_bridge_precreated_request_replays_verified_full_resend(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    original_full_resend = json.dumps(
        {"type": "response.create", "model": "gpt-5.6-sol", "input": ["complete history"]},
        separators=(",", ":"),
    )
    session = proxy_module._HTTPBridgeSession(
        key=proxy_module._HTTPBridgeSessionKey("prompt_cache", "retry-full-resend", None),
        headers={},
        affinity=proxy_module._AffinityPolicy(
            key="retry-full-resend",
            kind=proxy_module.StickySessionKind.PROMPT_CACHE,
            max_age_seconds=300,
        ),
        request_model="gpt-5.6-sol",
        account=cast(Account, SimpleNamespace(id="acct-stalled", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, _SilentUpstreamWebSocket()),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
    ...
    request_state = proxy_module._WebSocketRequestState(
        request_id="req-retry-full-resend",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=time.monotonic(),
        previous_response_id="resp-proxy-anchor",
        proxy_injected_previous_response_id=True,
        fresh_upstream_request_text=original_full_resend,
        fresh_upstream_request_is_retry_safe=True,
        awaiting_response_created=True,
        request_text='{"type":"response.create","previous_response_id":"resp-proxy-anchor","input":["delta"]}',
    )
    session.pending_requests.append(request_state)
    replacement_upstream = _RecordingUpstreamWebSocket()
    ...
    async def fake_reconnect(
        ...
        *,
        ...
        assert prefer_same_account is False
        assert require_exclusive_request is True
        target_session.account = cast(Account, SimpleNamespace(id="acct-recovery", status=AccountStatus.ACTIVE))
        target_session.upstream = replacement_upstream

    monkeypatch.setattr(proxy_module.ProxyService, "_reconnect_http_bridge_session_locked", fake_reconnect)

    assert await service._retry_http_bridge_precreated_request(session) is True
    assert replacement_upstream.sent_text == [original_full_resend]
    assert request_state.previous_response_id is None
    assert request_state.proxy_injected_previous_response_id is False
    assert request_state.request_text == original_full_resend
    assert request_state.replay_count == 1
    assert request_state.http_bridge_send_completed_at is not None


@pytest.mark.asyncio
async def test_retry_http_bridge_precreated_request_refuses_changed_send_generation(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    session = proxy_module._HTTPBridgeSession(
        key=proxy_module._HTTPBridgeSessionKey("prompt_cache", "retry-generation-change", None),
        headers={},
        affinity=proxy_module._AffinityPolicy(key="retry-generation-change"),
        request_model="gpt-5.6-sol",
        account=cast(Account, SimpleNamespace(id="acct-generation-old", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, _SilentUpstreamWebSocket()),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
    ...
    request_state = proxy_module._WebSocketRequestState(
        request_id="req-generation-change",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        ...
        http_bridge_send_completed_at=10.0,
    )
    session.pending_requests.append(request_state)
    replacement_upstream = _RecordingUpstreamWebSocket()
    ...
    async def fake_reconnect(
        ...
        *,
        ...
        del self, restart_reader, prefer_same_account
        assert require_exclusive_request is True
        target_session.upstream = replacement_upstream
        request_state.http_bridge_send_completed_at = 11.0
    ...
    assert (
        await service._retry_http_bridge_precreated_request(
            session,
            expected_request_tokens=frozenset({("req-generation-change", 10.0)}),
        )
        is False
    )
    assert replacement_upstream.sent_text == []
...
@pytest.mark.asyncio
async def test_retry_http_bridge_precreated_request_refuses_client_continuation(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    session = proxy_module._HTTPBridgeSession(
        key=proxy_module._HTTPBridgeSessionKey("prompt_cache", "retry-client-anchor", None),
        headers={},
        affinity=proxy_module._AffinityPolicy(key="retry-client-anchor"),
        request_model="gpt-5.6-sol",
        account=cast(Account, SimpleNamespace(id="acct-client-anchor", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, _SilentUpstreamWebSocket()),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
    ...
    request_state = proxy_module._WebSocketRequestState(
        ...
        awaiting_response_created=True,
        request_text='{"type":"response.create","previous_response_id":"resp-client-anchor"}',
    )
    session.pending_requests.append(request_state)
    reconnect = AsyncMock()
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_stream_masks_single_anonymous_previous_response_not_found(
    async_client,
    app_instance,
    monkeypatch,
):
    ...
    account_id = await _import_account(
        async_client,
        "acc_http_bridge_single_anon_previous_response_mask",
        "http-bridge-single-anon-previous-response-mask@example.com",
    )
    account = await _get_account(account_id)
    first_upstream = _FakeBridgeUpstreamWebSocket()
    recovered_upstream = _FakeBridgeUpstreamWebSocket()
    ...
    first = await async_client.post(
        ...
        json={
            ...
            "instructions": "Return exactly OK.",
            "input": "hello",
            "prompt_cache_key": "single-anonymous-previous-response-mask",
        },
    )
    assert first.status_code == 200
    first_body = first.json()
    ...
    assert "previous_response_not_found" not in json.dumps(second_events)
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_reconnects_after_foreign_previous_response_not_found(
    async_client,
    monkeypatch,
):
    _install_bridge_settings(monkeypatch, enabled=True)
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_stream_reconnects_after_foreign_previous_response_not_found(
    async_client,
    monkeypatch,
):
    _install_bridge_settings(monkeypatch, enabled=True)
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_stream_reconnects_after_anonymous_prev_nf_created_followup(
    app_instance,
    monkeypatch,
):
    _install_bridge_settings(monkeypatch, enabled=True)
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_ambiguous_send_retires_session_but_followup_recovers(
    async_client,
    app_instance,
    monkeypatch,
):
    ...
    assert first.json()["error"]["message"] == "HTTP bridge upstream send failed"
    assert "socket closed" not in first.json()["error"]["message"]
    assert second.status_code == 200
    assert second.json()["id"]
    assert connect_count == 2

    affinity = proxy_module._sticky_key_for_responses_request(
        proxy_module.ResponsesRequest.model_validate(payload),
        {},
        codex_session_affinity=False,
        openai_cache_affinity=True,
        openai_cache_affinity_max_age_seconds=300,
    ...
    assert affinity.key is not None
    session_key = proxy_module._HTTPBridgeSessionKey(
        affinity_kind="prompt_cache", affinity_key=affinity.key, api_key_id=None
    )
    async with service._http_bridge_lock:
...
async def test_prepare_http_bridge_request_preserves_existing_client_metadata(app_instance):
    ...
    assert first_request_state.input_item_count == len(input_items)
    assert first_request_state.input_full_fingerprint == proxy_module._fingerprint_input_items(input_items)
    assert second_request_state.input_item_count == first_request_state.input_item_count
    assert second_request_state.input_full_fingerprint == first_request_state.input_full_fingerprint
